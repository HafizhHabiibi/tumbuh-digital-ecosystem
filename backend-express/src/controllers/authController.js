    ...
    try {
        ...
        if (user.role === 'kader') {
            const [rows] = await db.query(
                `SELECT * FROM kader WHERE user_id = ?`, [user.id]
            )
            profil = rows[0]
        } else if (user.role === 'puskesmas') {
            const [rows] = await db.query(
                `SELECT * FROM puskesmas_user WHERE user_id = ?`, [user.id]
            )
            profil = rows[0]
            ...
            const [rows] = await db.query(
                `SELECT * FROM orang_tua WHERE user_id =?`, [user.id]
            )
            profil = rows[0]
        }
...
export const changePassword = async (req, res) => {
    try {
        ...
        const passwordMatch = await bcrypt.compare(password_lama, user.password_hash);
        if (!passwordMatch) {
...
export const forgotPassword = async (req, res) => {
    try {
        ...
        if (!user) {
            return success(res, null, 'Jika email terdaftar, link reset password akan dikirimkan')
        }

        const token = await PasswordResetModel.create(user.id);
        await mailerService.kirimEmailResetPassword(user.email, token);

        return success(res, null, 'Jika email terdaftar, link reset password akan dirikimkan');
...
export const resetPassword = async (req, res) => {
    try {
        ...
        if (!resetData) {
            return error(res, 'Link reset tidak valid atau sudah kadaluarsa, silahkan lakukan reques ulang', 400);
        }
        ...
        return success(res, null, 'Password berhasil direset, silahkan login ulang dengan password yang baru')
